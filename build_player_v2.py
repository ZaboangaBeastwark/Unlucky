...
function renderCharacterWizard(container) {
    container.innerHTML = `
        <div class="wizard-container glass-panel" style="padding: 2rem; max-width:1000px; margin: 0 auto;">
            <h2 style="font-family:'Crimson Text', serif; font-size:2rem; text-align:center; color:var(--accent-gold); margin-bottom:1rem;">Criação de Personagem Daggerheart</h2>
            <div id="wizard-progress" style="text-align:center; color:var(--text-muted); margin-bottom:2rem; font-size:0.9rem;"></div>
...
function step2HTML() {
    let ancHtml = `<div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(220px, 1fr)); gap:1rem; margin-bottom:2rem; max-height: 300px; overflow-y:auto; padding-right:10px;">`;
    for(const [aName, aText] of Object.entries(DH_ANCESTRIES_DATA)) {
        ancHtml += `
            <div class="glass-panel wiz-ancestry-card ${wizardState.ancestry === aName ? 'selected' : ''}" data-val="${aName}" style="padding:1rem; cursor:pointer; border:1px solid ${wizardState.ancestry === aName ? '#e78c3c' : 'var(--glass-border)'}">
                <div style="font-weight:bold; color:#e78c3c; margin-bottom:0.3rem;">${aName}</div>
                <div style="font-size:0.8rem; color:var(--text-muted); line-height:1.4;">${aText}</div>
            </div>
        `;
    }
    ancHtml += `</div>`;

    let comHtml = `<div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(220px, 1fr)); gap:1rem; margin-bottom:2rem; max-height: 300px; overflow-y:auto; padding-right:10px;">`;
    for(const [cName, cText] of Object.entries(DH_COMMUNITIES_DATA)) {
        comHtml += `
            <div class="glass-panel wiz-com-card ${wizardState.community === cName ? 'selected' : ''}" data-val="${cName}" style="padding:1rem; cursor:pointer; border:1px solid ${wizardState.community === cName ? '#3498db' : 'var(--glass-border)'}">
                <div style="font-weight:bold; color:#3498db; margin-bottom:0.3rem;">${cName}</div>
                <div style="font-size:0.8rem; color:var(--text-muted); line-height:1.4;">${cText}</div>
            </div>
        `;
    }
    comHtml += `</div>`;
    ...
    if (step === 1) {
        document.querySelectorAll('.wiz-class-card').forEach(card => {
            card.addEventListener('click', (e) => {
                document.querySelectorAll('.wiz-class-card').forEach(c => c.classList.remove('selected', 'highlight'));
        ...
        document.querySelectorAll('.wiz-weapon-card').forEach(card => {
            card.addEventListener('click', (e) => {
                document.querySelectorAll('.wiz-weapon-card').forEach(c => c.classList.remove('selected'));
                e.currentTarget.classList.add('selected');
        ...
        document.querySelectorAll('.wiz-armor-card').forEach(card => {
            card.addEventListener('click', (e) => {
                document.querySelectorAll('.wiz-armor-card').forEach(c => c.classList.remove('selected'));
                e.currentTarget.classList.add('selected');
...
async function submitCharacter() {
    // 8 is fallback fallback
    const finalEvasion = 8 + (INVENTORY_ITEMS.armors[wizardState.armor].evasion_mod || 0);
...
function renderCharacterSheet(char, container) {
    ...
    let attrHtml = '';
    for (let [k, v] of Object.entries(char.attributes)) {
    ...
    if (char.inventory.bag.length) {
        char.inventory.bag.forEach(i => invHtml += `<li><i class="fas fa-caret-right" style="color:var(--text-muted); font-size:0.8rem; margin-right:5px;"></i> ${i}</li>`);
    }
    invHtml += `</ul>`;
...
with open('./js/player.js', 'w', encoding='utf-8') as f:
    f.write(player_js_code)

print("player.js reconstructed for the massive 8-step wizard.")
